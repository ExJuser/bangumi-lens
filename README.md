...
Bangumi Lens 是一个面向 Bangumi 动画章节的评论区复盘工具。它可以解析公开的 Bangumi 单集页面，聚合评论、楼中楼回复、表情、点赞、单集评分、条目评分和少量公开网页线索，并调用兼容 OpenAI Chat Completions 的 DeepSeek API 生成结构化中文报告。
...
- 解析公开章节页，提取章节标题、所属条目、集数、简介、评论、回复、表情和点赞。
- 获取单集评分分布，并在可用时补充整部作品的条目评分作为背景参照。
- 根据回复数、表情、点赞、评论长度和信息量词汇计算评论权重。
